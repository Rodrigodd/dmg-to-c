...
impl LoweredDecomposedTimingModel {
    ...
    pub fn applied_facts(&self) -> &AppliedTimingFacts {
        &self.applied_facts
    }

    pub fn actual_verification(&self) -> &ActualDecompositionVerification {
        &self.actual_verification
    }
...
pub fn lower_design_with_decomposed_timing_and_generate_mode(
    ...
    let elaborated = elaborate_design(design, mode)?;
    let analysis = analyze_design_structural(&elaborated);
    lower_decomposed_timing_model_for_elaborated_design(&elaborated, &analysis)
}
...
pub fn lower_design_with_decomposed_timing_and_catalog_and_generate_mode(
    ...
    let flattened =
        crate::hierarchy::flatten_design_with_catalog_and_generate_mode(design, catalog, mode)?;
    let analysis = analyze_design_structural(&flattened);
    lower_decomposed_timing_model_for_elaborated_design(&flattened, &analysis)
}
...
"#,
    ...
    #[test]
    fn lowers_direct_transistor_kinds_without_normalizing_structure() {
        let lowered = lower_snippet(
  ...
  endspecify\n\
        ...
        assert_eq!(
            decomposed.actual_verification().checked_placements().len(),
            decomposed.decomposition().placements().len()
        );
        assert!(
            decomposed
                .assignment_provenance()
    ...
    fn decomposed_timing_smoke_covers_ao21() {
        ...
        assert_eq!(
            first.actual_verification().checked_placements().len(),
            first.decomposition().placements().len()
        );
