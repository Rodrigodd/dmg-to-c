...
use sv_to_sexpr::ir::{
    Cell, CellItem, DelayTuple, Expr, TimingExpr, TimingOperator, ValueOperator,
};
...
pub fn lower_repository_file(logical_path: &str) -> Cell {
    let input = fs::read_to_string(repository_root().join(logical_path))
...
pub fn assert_or_update_fixture(name: &str, extension: &str, actual: &str) {
    ...
    if std::env::var_os("UPDATE_LOWERING_GOLDENS").is_some() {
    ...
    assert_eq!(
        actual,
        expected,
        "lowering fixture {} changed",
        fixture.display()
    );
}

pub fn render_typed_ir(cell: &Cell) -> String {
