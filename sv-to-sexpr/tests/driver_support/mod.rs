...
use sv_to_sexpr::ir::{CellItem, Expr, LoweredModule, StrengthPair, TimingOperator, ValueOperator};
...
fn render_value_atom(expr: &Expr) -> String {
    format!("atom({})", value_atom(expr))
}

fn value_atom(expr: &Expr) -> &str {
    match expr {
        Expr::Atom(atom) if !atom.is_empty() => atom,
        Expr::Atom(_) => panic!("validated value atom must not be empty"),
